...
dat<- read.csv("Output/intrusion_continuous_set3B_output.csv", header=T)
...
p2<- ggplot(dat, aes(Generation, RpS, colour=Comp))+
  ...
  ylab("Recruits \n per spawner") + theme_bw() + facet_grid(~h2, labeller = labeller(h2=h2labs))
...
p4<- ggplot(dat, aes(Generation, Allele_freq, colour=Comp))+
  ...
  #geom_hline(yintercept=0, linetype = "dashed") +
  theme_bw() + facet_grid(~h2, labeller = labeller(h2=h2labs))
...
(p1 + p2)/(p3+p4)/(p5+p6) + plot_layout(guides = "collect") + plot_annotation(tag_levels = 'A') & theme(legend.position = 'bottom', legend.text = element_text(size=6), axis.title = element_text(size = 9)) & guides(colour=guide_legend(nrow=1)) & labs(colour=NULL)
