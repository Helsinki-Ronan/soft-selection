...
(p2+p3)/(p1+p8)/(p9+p7) + plot_layout(guides = "collect") + plot_annotation(tag_levels = 'A') & theme(legend.position = 'bottom', legend.text = element_text(size=6), axis.title = element_text(size = 9)) & guides(colour=guide_legend(nrow=1)) & labs(colour=NULL)
